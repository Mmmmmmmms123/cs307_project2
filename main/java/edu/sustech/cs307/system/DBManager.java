...
public class DBManager {
    ...
    public void showTables() {
        List<String> tableNames = new ArrayList<>(metaManager.getTableNames());
        int width=1;
        Logger.info(DBEntry.getStartEndLine(width,true));
        Logger.info("|" + StringUtils.center("TABLES",15,' ')+"|");
        Logger.info(DBEntry.getSperator(width));
        for (String name:tableNames) {
            Logger.info("|"+StringUtils.center(name,15,' ')+"|");
        }
        Logger.info(DBEntry.getStartEndLine(width,false));
    }

    //指定表的字段和类型
    public void descTable(String table_name) {
        if(!isTableExists(table_name)) {
            Logger.error("The table is not found:" + table_name);
            return;
        }

        TableMeta tableMeta;
        try {
            tableMeta = getMetaManager().getTable(table_name);
        } catch (DBException e) {
            Logger.error("Failed to fetch table: " + e.getMessage());
            return;
        }

        int width=2;
        Logger.info(DBEntry.getStartEndLine(width,true));
        Logger.info("|"+StringUtils.center("TABLE Field",15,' ')
        +"|"+StringUtils.center("Column Type",15,' ')+"|");
        Logger.info(DBEntry.getSperator(width));

        for (ColumnMeta col:tableMeta.columns_list) {
            Logger.info("|"+StringUtils.center(col.name,15,' ') +"|"+StringUtils.center(col.type.name(),15,' ')+"|");
            Logger.info(DBEntry.getSperator(width));
        }
