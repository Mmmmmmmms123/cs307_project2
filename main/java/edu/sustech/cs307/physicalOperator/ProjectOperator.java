...
public class ProjectOperator implements PhysicalOperator {
    ...
    public ArrayList<ColumnMeta> outputSchema() {
        ArrayList<ColumnMeta> projected = new ArrayList<>();
        ArrayList<ColumnMeta> childSchema = child.outputSchema();

        for (TabCol tabCol : outputSchema) {
            boolean found = false;
            for (ColumnMeta meta : childSchema) {
                if ((tabCol.getTableName().equals("*") || tabCol.getTableName().equals(meta.tableName)) &&
                        tabCol.getColumnName().equals(meta.name)) {
                    projected.add(meta);
                    found = true;
                    break;
                }
            ...
            if (!found) {
                throw new RuntimeException("Column not found in child schema: " + tabCol);
            }
        }
        return projected;
    }
